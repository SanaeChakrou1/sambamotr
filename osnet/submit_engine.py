...
class Submitter:
    def __init__(self, dataset_name: str, split_dir: str, seq_name: str, outputs_dir: str, model: nn.Module,
        ...
        self.tracker = RuntimeTracker(det_score_thresh=det_score_thresh, track_score_thresh=track_score_thresh,
                                      ...
                                      use_reid=use_reid, reid_thresh=reid_thresh,
                                      reid_model=reid_model, reid_path=reid_path)
    ...
    def run(self):
        ...
        for i, ((image, ori_image), info) in enumerate(dataloader):
            ...
            # Following MeMOTR, we do not use this.
            if self.use_motion:
            ...
            if self.visualize:
                ...
                os.system(f"mv ./outputs/visualize_tmp/sambamotr/ ./outputs/visualize_tmp/frame_{idx+1}/")
...
def submit(config: dict):
    ...
    reid_model = config.get("REID_MODEL", 'osnet_x1_0')
    reid_path = config.get("REID_PATH", None)
